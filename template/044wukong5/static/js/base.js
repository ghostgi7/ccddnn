...
document.writeln('	<div class="area">\u8b66\u544a\uff1a\u5982\u60a8\u672a\u6eff\x31\x38\u6b72\uff0c\u6216\u672a\u6eff\u7576\u5730\u6cd5\u5f8b\u8a31\u53ef\u7684\u6cd5\u5b9a\u5e74\u9f61\uff0c\u5efa\u8b70\u60a8\u96e2\u958b\u672c\u7ad9\uff01</div>');
...
baseready(function() {
    ...
    if (getCookie("khspapp") != 0) {
        ...
        appTips.innerHTML = '<div class="app-wrap"><div class="close-bg"></div><div class="close" onclick="closeTips();return false;"></div><div class="icon"></div><div class="text"><b>\u5feb\u6d3b\u89c6\u9891</b><br>海量免费高清视频</div><div class="download"><a href="' + appUrl + '" onclick="closeTips();" target="_blank">立即下载</a></div></div>';
...
function makhtml(str) {
    ...
    if (str.indexOf('midSpons') != -1) {
        ...
        for (var i = 0; i < midData.length; i++) {
            midSpons += '			<li><a href="' + midData[i].url + '" target="_blank" rel="nofollow"><img src="/static/images/empty_288_144.jpg" /><img class="spon-img" src="' + midData[i].img + '" alt="" /><span>\u5e7f\u544a</span></a></li>';
        }
        midSpons += '		</ul>';
        midSpons += '	</div>';
        midSpons += '</div>';
...
; (function() {
    ...
    SliderQfl.prototype.createDom = function(el, sliderData, sliderClass) {
        ...
        var spotEl = '';
        for (var i = 0; i < len; i++) {
            imgEl += '<div class="broadcastMe-item">' + '<a href="' + sliderData[i].url + '" target="_blank" rel="nofollow"><img src="' + this.rowImg(sliderData[i]) + '" /></a></div>'
            if (i == 0) {
        ...
        str += '<div class="broadcastMe"><div class="broadcastMe-list ' + sliderClass + '">' + imgEl + '<div class="broadcastMe-item"><a href="' + sliderData[0].url + '" target="_blank" rel="nofollow"><img src="' + this.rowImg(sliderData[0]) + '" /></a></div></div>' + '<div id="broadcastMe-tool" class="broadcastMe-tool">' + spotEl + '</div><div id="broadcastMe-btn-left" class="broadcastMe-btn broadcastMe-btn-left"> &lt; </div>' + '<div id="broadcastMe-btn-right" class="broadcastMe-btn broadcastMe-btn-right"> &gt;</div></div>'el.innerHTML = str;
    }
    ...
    SliderQfl.prototype.bindEvent = function() {
        var self = this;
        addEvent(self.broadcastMeBtnLeft, 'click',
        function() {
            self.autoMove('toLeft', true);
        });
        addEvent(self.broadcastMeBtnRight, 'click',
        function() {
            self.autoMove('toRight', true);
        }) addEvent(self.broadcastMe, 'mouseenter',
        function() {
            clearTimeout(self.timer);
        }) addEvent(self.broadcastMe, 'mouseleave',
        function() {
            self.timer = setTimeout(self.autoMove.bind(self), 3000);
        }) for (var i = 0; i < this.broadcastMeSpot.length; i++) {; (function(i) {
                addEvent(self.broadcastMeSpot[i], 'click',
                function(e) {
                    self.lock = false;
                    clearTimeout(self.timer);
                    self.curIndex = i;
                ...
                }) addEvent(self.broadcastMeSpot[i], 'touchend',
                function(e) {
                    self.lock = false;
                    clearTimeout(self.timer);
                    self.curIndex = i;
