...
var MAC={
    ...
    'UserAgent' : function(){
        ...
        return {
            'mobile': !!ua.match(/AppleWebKit.*Mobile.*/), //鏄惁涓虹Щ鍔ㄧ粓绔?            'ios': !!ua.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/), //ios缁堢
            'android': ua.indexOf('Android') > -1 || ua.indexOf('Linux') > -1, //android缁堢鎴栬€卽c娴忚鍣?            'iPhone': ua.indexOf('iPhone') > -1 || ua.indexOf('Mac') > -1, //鏄惁涓篿Phone鎴栬€匭QHD娴忚鍣?            'iPad': ua.indexOf('iPad') > -1, //鏄惁iPad
            'trident': ua.indexOf('Trident') > -1, //IE鍐呮牳
            'presto': ua.indexOf('Presto') > -1, //opera鍐呮牳
            'webKit': ua.indexOf('AppleWebKit') > -1, //鑻规灉銆佽胺姝屽唴鏍?            'gecko': ua.indexOf('Gecko') > -1 && ua.indexOf('KHTML') == -1, //鐏嫄鍐呮牳
    ...
    'Ulog':{
        ...
        'Click':function(){
            $('body').on('click', 'a.mac_ulog', function(e){
                //鏄惁闇€瑕侀獙璇佺櫥褰?                if(MAC.User.IsLogin == 0){
                    MAC.User.Login();
                    return;
                }
...
$(function(){
    ...
    //楠岃瘉鐮佸垵濮嬪寲
    MAC.Verify.Init();
    //鍒嗛〉璺宠浆鍒濆鍖?    MAC.PageGo.Init();
    //鐢ㄦ埛閮ㄥ垎鍒濆鍖?    MAC.User.Init();
    //椤跺拰韪╁垵濮嬪寲
    MAC.Digg.Init();
    //璇勫垎鍒濆鍖?    MAC.Score.Init();
    //鏄熸槦璇勫垎鍒濆鍖?    MAC.Star.Init();
    //鐐瑰嚮鏁伴噺
    MAC.Hits.Init();
    //鍘嗗彶璁板綍鍒濆鍖?    MAC.Ulog.Init();
    //鑱旀兂鎼滅储鍒濆鍖?    MAC.Suggest.Init('.mac_wd',1,'');
    //瀹氭椂浠诲姟鍒濆鍖?    MAC.Timming();
});
